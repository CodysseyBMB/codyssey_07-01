...
현재 FastAPI 애플리케이션, Jinja2 화면, 사용자 테이블 초기화와 회원가입·로그인
기능까지 구성되어 있습니다. 채팅과 AI API 연동은 후속 Issue에서 구현할
예정입니다.
...
실행 전에 다음 명령으로 세션 서명 키를 생성하고, 출력된 값을 `.env`의
`SESSION_SECRET=` 뒤에 입력합니다.

```bash
python3 -c "import secrets; print(secrets.token_hex(32))"
```

값을 입력한 뒤 애플리케이션을 실행합니다.
...
브라우저에서는 다음 순서로 인증 흐름을 확인할 수 있습니다.

1. `/signup`에서 아이디와 비밀번호로 계정을 생성합니다.
2. 가입 완료 후 이동한 `/login`에서 같은 정보로 로그인합니다.
3. `/`에 로그인한 아이디가 표시되는지 확인합니다.
4. 로그아웃한 뒤 `/`가 비로그인 화면으로 돌아오는지 확인합니다.
...
- `pip install -r requirements-dev.txt`가 오류 없이 완료됐는지 확인합니다.
- `8000` 포트를 사용하는 다른 프로그램이 있는지 확인합니다.

## 환경 변수 관리

- `SESSION_SECRET`: 세션 쿠키 서명에 사용하는 32자 이상의 필수 비밀값입니다.
- `SESSION_HTTPS_ONLY`: 로컬 HTTP 환경에서는 `false`, HTTPS 배포 환경에서는
  `true`로 설정합니다.

`SESSION_SECRET`이 없거나 32자보다 짧으면 애플리케이션은 시작되지 않습니다.
실제 비밀값은 `.env.example`이 아닌 `.env`에만 작성하며 `.env`는 Git에
포함하지 않습니다.

## 인증 방식

- 아이디는 앞뒤 공백을 제거하고 소문자로 변환한 뒤 영문 소문자, 숫자,
  하이픈, 밑줄로 구성된 5~20자인지 검사합니다.
- 비밀번호는 공백 없이 영문 대소문자, 숫자, ASCII 특수문자로 구성된
  8~16자인지 검사하며 숫자만으로 된 비밀번호는 허용하지 않습니다.
- 비밀번호 원문은 저장하지 않고 Argon2id 해시만 `users.password_hash`에
  저장합니다.
- 로그인 상태는 DB 세션 테이블 대신 8시간 동안 유효한 서명 쿠키로
  유지합니다. 쿠키에는 사용자 ID와 CSRF 토큰만 담습니다.
- 회원가입, 로그인, 로그아웃 요청에는 CSRF 토큰 검증을 적용합니다.

현재 로그인 요청의 처리 흐름, 세션 방식의 한계와 향후 확장 방향은
[인증 구조와 발전 방향](docs/AUTHENTICATION.md)에 정리되어 있습니다.
